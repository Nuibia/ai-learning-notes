...
type ModelDecision =
  ...
  | { type: "tool_call"; callId: string; name: "list_projects" }
  | { type: "tool_call"; callId: string; name: "get_project"; id: string }
  | { type: "tool_call"; callId: string; name: "create_project"; projectName: string };

type ToolResult =
  | { callId: string; kind: "projects"; projects: string[] }
  | { callId: string; kind: "created"; projectName: string };

type RuntimeOutput =
  | { event: "tool.result"; detail: ToolResult }
  | { event: "tool.error"; detail: { callId: string; code: "PROJECT_NOT_FOUND" } }
  | { event: "tool.refused"; detail: { callId: string; code: "ROLE_VIEWER" } }
  | { event: "runtime.confirmation_required"; detail: { callId: string; operation: "create_project" } }
  | { event: "runtime.no_tool"; detail: ModelDecision };

function decide(userInput: string): ModelDecision {
...
function continueAfterTool(result: ToolResult) {
...
function receive(decision: ModelDecision, confirmed: boolean, role: string): RuntimeOutput {
